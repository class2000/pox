...
type SdnControlInput struct {
	// Use pointers generated by protoc for direct compatibility
...
type ConsensusResult struct {
	TxHash         []byte                `json:"tx_hash"`         // Hash of the original SdnControlInput transaction
	Success        bool                  `json:"success"`         // True if consensus was reached and validated
	FinalAction    *pbftconsensus.Action `json:"final_action"`    // The final action (this is already the proto type)
	ResultMessage  string                `json:"result_message"`  // e.g., "OK", "Validation Failed", "Timeout"
	ReportingNode  ed25519.PublicKey   `json:"reporting_node"`  // Public key of the node sending the result
	// Consider adding a signature from the ReportingNode for integrity
	// Signature      []byte                `json:"signature"`
}

// ToByte serializes ConsensusResult using JSON for network transmission.
// This implements the Payload interface.
...
// GetHashPayload for a ConsensusResult returns the base64 encoded string of the original TxHash it refers to.
// This implements the Payload interface.
func (cr ConsensusResult) GetHashPayload() string {
	return base64.StdEncoding.EncodeToString(cr.TxHash)
}

// GetProposer for a ConsensusResult returns the public key of the node that reported this result.
// This implements the Payload interface.
