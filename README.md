...
*   **POX SDN Controller System (in `pox/ext/pbft_pox_app.py` and standard POX files):**
...
Sequence diagrams illustrating the system's operational flows are available in the `plantuml/` directory:
*   `plantuml/topology.txt`: Visualizes the Topology Update (Link Event) flow.
*   `plantuml/packetin_packetout.txt`: Visualizes the Packet Processing (Packet-In to Packet-Out) flow.

These can be viewed using any PlantUML renderer. A convenient online option is the official PlantUML Web Server:
[https://www.plantuml.com/plantuml/uml](https://www.plantuml.com/plantuml/uml)
(Simply copy and paste the content of the `.txt` files into the web interface.)
...
*   **Python 3** — required for `generate_docker_compose.py`, POX, `real_time_ui.py`, and `/Users/andreidanila/Desktop/pox/tests/test.py`  
*   **Python packages** — `streamlit`, `pandas`, `streamlit-agraph`, `pyyaml`, `seaborn`, `scipy` (install via `pip install -r requirements.txt`)
...
*   **(Optional) Java Runtime Environment:** If you wish to run PlantUML locally to generate diagrams from the `.txt` files.
...
1.  **Clone this Repository (which includes POX and your Sabine fork):**
    ...
    ```
    .
    ├── docker-compose.generated.yml   # Auto-generated by generate_docker_compose.py
    ├── Dockerfile                     # Container config for POX and components
    ├── generate_docker_compose.py     # Script to generate Docker Compose config
    ├── mermaid/                       # (Optional) Diagram sources in Mermaid format
    ├── plantuml/                      # PlantUML diagrams (e.g., packet flows, topology)
    ├── pox/                           # POX controller core + custom extensions (e.g. pbft_pox_app.py)
    ├── README.md                      # Project documentation
    ├── real_time_ui.py                # Streamlit-based UI for visualizing network activity
    ├── requirements.txt               # Python dependencies
    ├── Sabine_ODL_Fork/              # Go-based PBFT implementation
    ├── screenshots/                   # UI and network state snapshots
    ├── tests/                         # Includes testing scripts and analysis tools
    ```
...
1.  **Generate Docker Compose Configuration:**
    ...
    ```
    This creates `docker-compose.generated.yml`. Ensure `pbft-node-0` (or another node) in this generated file has the `--httpChain` flag and port mapping if you intend to use the Streamlit UI (`real_time_ui.py`).
...
5.  **Visualize Blockchain Data & Topology (Streamlit UI):**
    ...
    ```
    Open the URL provided by Streamlit in your browser. Configure the "Node IP" (usually `127.0.0.1` or `localhost`) and "Node HTTP Port" (e.g., `8080`, matching what you mapped for `pbft-node-0`). The UI will fetch blockchain data from the specified PBFT node and display the consensus-agreed switch topology derived from committed `LinkEventInput` transactions.
...
6.  **Perform Network Tests (e.g., Ping in Mininet):**
    ...
    ```
    Observe logs in `pox-primary`, `pbft-dealer`, and `pbft-node-X` containers to see the PacketIn processing flow. The Streamlit UI (`real_time_ui.py`) should remain stable or reflect only link changes, as PacketIn events don't alter the switch fabric topology recorded on the blockchain.
...
7.  **Analyze Results (After Test Completion):**
    ...
    *   Inspect blockchain data via the Streamlit UI (`real_time_ui.py`) or the node's direct HTTP endpoint (`http://localhost:8080/`).
